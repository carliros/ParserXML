...
sem_Node :: Node  ->
            T_Node 
sem_Node (NTag _tag _attributes )  =
    (sem_Node_NTag _tag _attributes )
...
sem_Node_NTag :: String ->
                 ([(String, String)]) ->
                 T_Node 
sem_Node_NTag tag_ attributes_  =
    (\ _lhsInoderoot ->
         (let _lhsOppdocs :: ([PP_Doc])
              _lhsOppdocs =
                  if _lhsInoderoot
                  then [ text  "---NTag" >#< text tag_ ]
                  else (text "|") :
                       [text "+---NTag" >#< text tag_ ] ++
                       (map (\(nm,vl) -> text "|" >#< text nm >#< text "=" >#< text (show vl)) attributes_)
          in  ( _lhsOppdocs)))
